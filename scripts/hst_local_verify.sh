...
"$PYTHON_BIN" trainer/train_hst_pretrain.py --method residual_structured_tst --run_name verify_residual_structured --output_dir "$PROJECT_ROOT/hst_runs/verify_residual_structured" --superpose_size 4 --loss_mode repeated_ce --order_alpha 0.05 --hier_alpha 0.05 --recovery_ratio 0.34 "${COMMON_ARGS[@]}"
"$PYTHON_BIN" trainer/train_hst_pretrain.py --method sparse_anchor_residual_tst --run_name verify_sparse_anchor_residual --output_dir "$PROJECT_ROOT/hst_runs/verify_sparse_anchor_residual" --superpose_size 4 --loss_mode sparse_anchor_residual --order_alpha 0.05 --hier_alpha 0.0 --anchor_slot_idx 0 --residual_codebook_size 64 --sar_gate_threshold 0.05 --residual_loss_weight 0.5 --recovery_ratio 0.34 "${COMMON_ARGS[@]}"
